...
export default function MapClient({ focusedPropertyName }: { focusedPropertyName?: string }) {
  ...
  const locations = [
    ...
    { name: "Pamoja Farm Villas", pos: [-3.3000, 35.6666], desc: "Karatu Highlands." },
  ...
  // Default center on Northern Tanzania
  let centerPosition: [number, number] = [-3.0000, 35.5000];
  let zoomLevel = 7;

  const focusedLoc = focusedPropertyName 
    ? locations.find(l => l.name.toLowerCase() === focusedPropertyName.toLowerCase())
    : null;

  if (focusedLoc) {
    centerPosition = focusedLoc.pos as [number, number];
    zoomLevel = 11;
  }
  ...
  useEffect(() => {
    ...
    locations.forEach(loc => {
      const isFocused = focusedLoc && loc.name === focusedLoc.name;
      ...
      const marker = L.marker(loc.pos as [number, number], { icon: customIcon })
        .addTo(map)
  ...
  }, [focusedPropertyName]); // Re-run if focus changes
